...
int main(int argc, char** argv) {
	...
	Task::setDelimiter(argv[6][0]);
	CustomerOrder::setDelimiter(argv[6][0]);
	Item::setDelimiter(argv[6][0]);
	...
	AssemblyLine assemblyLine(argv[7]);
...
void loadFromFile<AssemblyLine, Processor>(const char* fileName, AssemblyLine& manager, std::ostream& os) {
	...
	while (file) {
		...
		if (file) {
			try {
				std::unique_ptr<Processor> entry(new Processor(record, inspect, manager.getLogFile()));
				if (!entry->getName().empty())
					manager.push_back(std::move(*entry));
			}
			catch (const std::string& msg) {
